            ...
            if focusTimestamp != 0:
                ...
                if slot is not None:
                    if slot["Stop"] > int(time.time()):
                        record = xbmcgui.Dialog().yesno("Set Record", "Would you like to record %s on %s @ %s?" % (
                            slot["Name"], channel["Name"], datetime.datetime.fromtimestamp(slot["Start"]).strftime('%I:%M %p')))
                        log('::onAction() Dialog result %s' % record)

                        if record:
                            recURL = 'plugin://plugin.video.sling/?mode=tryRecord&channel=%s&start=%s' % (
                                channel["ID"], slot["Start"])
